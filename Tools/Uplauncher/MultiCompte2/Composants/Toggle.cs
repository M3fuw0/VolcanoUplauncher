...
namespace Uplauncher.MultiCompte2.Composants
{
    class Toggle : Panel
    {
		...
		public GraphicsPath RoundRec(int X, int Y, int Width, int Height, int diameter)
		{
			...
			checked
			{
				graphicsPath.AddLine(X + (int)Math.Round(diameter / 2.0), Y, X + Width - (int)Math.Round(diameter / 2.0), Y);
				rect.X = rectangleF.Right - diameter;
				graphicsPath.AddArc(rect, 270f, 90f);
				graphicsPath.AddLine(X + Width, Y + (int)Math.Round(diameter / 2.0), X + Width, Y + Height - (int)Math.Round(diameter / 2.0));
				rect.Y = rectangleF.Bottom - diameter;
				graphicsPath.AddArc(rect, 0f, 90f);
				graphicsPath.AddLine(X + (int)Math.Round(diameter / 2.0), Y + Height, X + Width - (int)Math.Round(diameter / 2.0), Y + Height);
				rect.X = rectangleF.Left;
				graphicsPath.AddArc(rect, 90f, 90f);
				graphicsPath.AddLine(X, Y + (int)Math.Round(diameter / 2.0), X, Y + Height - (int)Math.Round(diameter / 2.0));
				return graphicsPath;
			}
		}
	}

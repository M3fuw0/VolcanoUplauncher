...
namespace DiscordRPC.RPC
{
	...
	internal class RpcConnection : IDisposable
    {
        ...
        public RpcConnection(string applicationID, int processID, int targetPipe, INamedPipeClient client,
            uint maxRxQueueSize = 128, uint maxRtQueueSize = 512)
        {
        ...
        private void ProcessFrame(EventPayload response)
        {
            ...
            if (State == RpcState.Connecting)
                if (response.Command == Command.Dispatch && response.Event.HasValue &&
                    response.Event.Value == ServerEvent.Ready)
                {
        ...
        private void EstablishHandshake()
        {
            ...
            if (!namedPipe.WriteFrame(new PipeFrame(Opcode.Handshake,
                    new Handshake { Version = VERSION, ClientID = applicationID })))
            {
        ...
        private void SendHandwave()
        {
            ...
            //Send the handwave
            if (!namedPipe.WriteFrame(new PipeFrame(Opcode.Close,
                    new Handshake { Version = VERSION, ClientID = applicationID })))

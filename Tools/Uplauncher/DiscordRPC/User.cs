...
namespace DiscordRPC
{
	...
	public class User
    {
	    ...
	    public enum AvatarFormat
        {
	        ...
	        ///     <para>
	        ///         Losses format that supports transparent avatars. Most recommended for stationary formats with wide support
	        ///         from many libraries.
	        ///     </para>
	        /// </summary>
	        PNG,
	        ...
	        ///     <para>
	        ///         Animated avatars that Discord Nitro users are able to use. If the user doesn't have an animated avatar, then
	        ///         it will just be a single frame gif.
	        ///     </para>
	        /// </summary>
	        GIF //Gif, as in gift. 
        }
	    ...
	    /// <summary>
	    ///     The avatar hash of the user. Too get a URL for the avatar, use the
	    ///     <see cref="GetAvatarURL(AvatarFormat, AvatarSize)" />. This can be null if the user has no avatar. The
	    ...
	    /// <summary>
	    ///     Gets a URL that can be used to download the user's avatar. If the user has not yet set their avatar, it will return
	    ///     the default one that discord is using. The default avatar only supports the <see cref="AvatarFormat.PNG" /> format.
	    /// </summary>
	    ...
	    public string GetAvatarURL(AvatarFormat format, AvatarSize size)
        {
            ...
            if (string.IsNullOrEmpty(Avatar))
            {
                ...
                if (format != AvatarFormat.PNG)
                    throw new BadImageFormatException("The user has no avatar and the requested format " + format +
                                                      " is not supported. (Only supports PNG).");

                //Get the endpoint
                var descrim = Discriminator % 5;
                endpoint = $"/embed/avatars/{descrim}";
            }

            //Finish of the endpoint
            return string.Format("https://{0}{1}{2}?size={3}", CdnEndpoint, endpoint, GetAvatarExtension(format),
                (int)size);
        }

...
namespace DiscordRPC.IO
{
	...
	public interface INamedPipeClient : IDisposable
    {
	    ...
	    /// <summary>
	    ///     Attempts to connect to the pipe. If 0-9 is passed to pipe, it should try to only connect to the specified pipe. If
	    ///     -1 is passed, the pipe will find the first available pipe.
	    /// </summary>
	    /// <param name="pipe">
	    ///     If -1 is passed, the pipe will find the first available pipe, otherwise it connects to the pipe that
	    ///     was supplied
	    /// </param>
	    /// <returns></returns>
	    ...
	    /// <summary>
	    ///     Reads a frame if there is one available. Returns false if there is none. This should be non blocking (aka use a
	    ///     Peek first).
	    /// </summary>

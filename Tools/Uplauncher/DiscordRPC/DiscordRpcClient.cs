    ...
    public sealed class DiscordRpcClient : IDisposable
    {
        ...
        public void Respond(JoinRequestMessage request, bool acceptRequest)
        {
            ...
            connection.EnqueueCommand(
                new RespondCommand { Accept = acceptRequest, UserID = request.User.ID.ToString() });
        }
        ...
        public void SetPresence(RichPresence presence)
        {
            ...
            else
            {
                ...
                if (presence.HasParty() && presence.Party.Max < presence.Party.Size)
                    throw new BadPresenceException(
                        "Presence maximum party size cannot be smaller than the current size.");
        ...
        ///         however, then this event is not invoked untill <see cref="Invoke" /> and will be on the calling thread.
        ///     </para>
        /// </summary>
        ...
        /// <summary>
        ///     Called when a error has occured during the transmission of a message. For example, if a bad Rich Presence payload
            ...
            INamedPipeClient client = null)
        {
            ...
            connection = new RpcConnection(ApplicationID, ProcessID, TargetPipe, client ?? new ManagedNamedPipeClient(),
                autoEvents ? 0 : 128U)
            {
        ...
        /// <summary>
        ///     Updates the <see cref="Party.Size" /> of the <see cref="CurrentPresence" /> and sends the update presence to
        ...
        private void SubscribeToTypes(EventType type, bool isUnsubscribe)
        {
            ...
            if ((type & EventType.Spectate) == EventType.Spectate)
                connection.EnqueueCommand(new SubscribeCommand
                    { Event = ServerEvent.ActivitySpectate, IsUnsubscribe = isUnsubscribe });

            if ((type & EventType.Join) == EventType.Join)
                connection.EnqueueCommand(new SubscribeCommand
                    { Event = ServerEvent.ActivityJoin, IsUnsubscribe = isUnsubscribe });

            if ((type & EventType.JoinRequest) == EventType.JoinRequest)
                connection.EnqueueCommand(new SubscribeCommand
                    { Event = ServerEvent.ActivityJoinRequest, IsUnsubscribe = isUnsubscribe });
        }
